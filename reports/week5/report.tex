...
For each of the 17 rooms, we have collected the Wi-Fi signal strengths
of all the access points. The rooms and the location probability
of the user for each of the rooms is drawn as a rough not-to-scale map,
which is shown on the screen as part of the GUI of the Android application.
See \autoref{fig:screenshot}.

In total we have collected \textbf{3300} scans for the WiFi access points on three different days.
...
Wi-Fi signals whose $SSID$ is either ``TUvisitor'', ``tudelft-dastud'',
or ``Conferentie-TUD'' are ignored.
This is because most access points of the TU Delft sent out four different
$SSID$'s (the fourth is ``eduroam'') with different $BSSID$'s.
...
While we achieved a pretty good accuracy with pure bayesian filters (see performance evaluation section),
the \emph{Coffee} room suggested that we could improve the accuracy by taking the probability based on the
number of access points that are visible in a room into account.
The normal distribution $N_{\#,Room}$ is constructed using the previously collected training data.
This gives the distribution of number of AP's in a scan based on the $\mu$ and $\sigma$ of the number of AP's per room in the training data.
This helped in rooms where not a lot of signals were measurable, such as the Coffee room. Thus we can append the following to the previous algorithm:
\\
\begin{algorithmic}
...
\begin{figure}
  \centering
    \includegraphics[width=0.36\textwidth]{correctness_perroom}
    \caption{Correct localizations per room.}
    \label{fig:correctness-perroom}
\end{figure}

\begin{figure}
  \centering
    \includegraphics[width=0.36\textwidth]{iterations_needed}
    \caption{Total amount of correct and certain results per number of iterations performed.}
    \label{fig:iterations-needed}
\end{figure}

\begin{figure}
  \centering
    \includegraphics[width=0.36\textwidth]{distribution}
...
\begin{figure}
  \centering
    \includegraphics[width=0.36\textwidth]{graph-numberAPperRoom}
