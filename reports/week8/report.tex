...
\begin{algorithmic}
	...
	\ForAll{$Room$}
		...
		\State $\mathbf{loc}_{Room} \gets \mathbf{loc}_{Room} \cdot p $
	\EndFor
	\State $\mathbf{loc} = \text{normalize}\left(\mathbf{loc}'\right)$
\end{algorithmic}
...
Furthermore, it proved to be difficult to reliably classify activities with small windows of around 60 samples.
