...
Initially the Google Cloud Messaging (GCM) service was used, but this proved to be slow. Because of this reason, we switched to using using a simple direct TCP socket connection. Response time was shown to be much faster; updating the location probabilities takes on average about $1.6$~seconds. Furhermore, the approach using TCP sockets was easier to implement than GCM.
